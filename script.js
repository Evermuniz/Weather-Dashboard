...
function displayHistory() { //this function will display the search history and create buttons
  var list = localStorage.getItem("city"); //grabbing values saved in local storage
  if (list) { //if local storage is not empty then parse the data
    searched = JSON.parse(list);
  }

  for (var i = 0; i < searched.length; i++) { //for loop to repeat for as many cities saved in the searched array
    ...
    items.addEventListener("click", function () { //adding event listener to each button
...
function getWeather() { //the first function triggered when a user searches for a city
...
function coordinates(search) { //this function will return the coordinates for the selected city. The parameter comes from getWeather function or displayHistory
  var coord = "https://api.openweathermap.org/geo/1.0/direct?q=" + search + "&limit=5&appid=f6302d4de9dd9a1785f80b5cb10cfdfc"; //using the search parameter (city name) to construct the API URL to retrieve city coordinates
  fetch(coord) //making a fetch call using the URL built above
    .then(function (response) { //return the response in json format
      return response.json();
    })
    .then(function (data) { //here we can access the data
...
function weather(search) { //using the data returned in the first array as a parameter from the coordinates function, this function will return the forecast
  ...
  var weatherAPI = "https://api.openweathermap.org/data/2.5/forecast?lat=" + lat + "&lon=" + lon + "&units=imperial&appid=f6302d4de9dd9a1785f80b5cb10cfdfc"; //using the lat & long values to construct the API URL to retrieve forecast
...
function fiveDayForecast(data) { //this function will render the future five day forecast for a given city, the parameter used is from the weather function
  ...
  for (var i = 1; i < 6; i++) {
    ...
    var fiveIcon = document.createElement("img"); //create a new image element
...
function history(city) { //this function will update the saved search history, the parameter used is passed from weather function
    if (!searched.includes(city)) { //if the searched array doesn't contain the city name then perform the function
  searched.push(city); //add the city to the searched array
  localStorage.setItem("city", JSON.stringify(searched));} //add the searched array to local storage in json.stringify format
